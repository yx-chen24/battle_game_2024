...
Tank_yx_chen::Tank_yx_chen(GameCore *game_core, uint32_t id, uint32_t player_id)
...
void Tank_yx_chen::Update() {
  TankMove(3.0f, glm::radians(180.0f));
  TurretRotate();
  Fire();
}

void Tank_yx_chen::TankMove(float move_speed, float rotate_angular_speed) {
...
void Tank_yx_chen::TurretRotate() {
  auto player = game_core_->GetPlayer(player_id_);
  if (player) {
    auto &input_data = player->GetInputData();
...
bool Tank_yx_chen::IsHit(glm::vec2 position) const {
...
const char *Tank_yx_chen::UnitName() const {
  return "yx-chen_tank";
}

const char *Tank_yx_chen::Author() const {
  return "yx-chen";
}
}  // namespace battle_game::unit
